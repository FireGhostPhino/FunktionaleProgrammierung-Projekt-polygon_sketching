...
It takes a message and the current model
Match executes the corresponding code
*)
let updateModel (msg : Msg) (model : Model) = 
    ...
    | AddPoint coord ->
        ...
        | None ->
        // No polygon exists yet: start a new one with new vertex
            { model with
                currentPolygon = Some [coord]}
        | Some poly ->
        // polygon already started being drawn: add the new vertex to existing polygon
        // prepend for efficiency
            { model with
                currentPolygon = Some (coord :: poly) }
    | FinishPolygon ->
        match model.currentPolygon with
        | None ->
        // No polygon to finish: ignore message
            model
        | Some poly ->
        // Finish the current polygon: 
        // create new finishedpolygons list with finishedpolygons so far and current one
        // and reset current polygon
            match poly with
            | [] | [_] ->
                // Not enough points to form a polygon
                model
            | firstPoint :: _ ->
                // Close the polygon by connecting last and first point
                let closedPoly = poly @ [firstPoint]
                { model with 
                    finishedPolygons = closedPoly :: model.finishedPolygons
                    currentPolygon = None }
    | _ ->
        model
...
(*
Wraps the core update function with undo and redo
Undo and redo function by storing previous and future models
Only references to old models are stored
*)
let addUndoRedo (updateFunction : Msg -> Model -> Model) (msg : Msg) (model : Model) =
    ...
    | Undo -> 
        ...
        | None ->
        // No previous state exists: nothing to undo
            model
        | Some previous ->
        // Previous state exists: restore previous model state as current one
        // and store current-until-now as future for redo
            { previous with 
                future = Some model}
    | Redo -> 
        match model.future with
        | None -> 
        // No future state exists: nothing to redo
            model
        | Some next ->
        // Future state exists: restore future model state as current one
        // and store current-until-now as previous for undo
            { next with 
                future = Some model}
    | _ -> 
        // use the provided update function for all remaining messages
        // Apply normal application update:
        // store current model as past one for undo and clear future (for redo)
...
let render (model : Model) (dispatch : Msg -> unit) =
    ...
    // Render all finished polygons in green
    ...
    let currentPolygon =
        ...
        | Some p -> 
            ...
            // Render the currently drawn polygon in red
            | None -> 
                viewPolygon "red" p
            // If a mouse position exists, a preview line to the mouse is shown in addition
